...
def classify_task(prompt: str) -> TaskCategory:
    ...
    if any(kw in p for kw in [
        ...
        "formula", "sum", "add", "subtract", "multiply", "divide",
        "+", "-", "*", "/", "%",
    ]):
    ...
    # Code generation
    if any(kw in p for kw in [
        "function", "def ", "class ", "import ", "code",
        "implement", "write a program", "script",
        "algorithm", "function that", "html", "javascript",
    ...
    if any(kw in p for kw in [
        ...
        "infer", "conclusion", "hypothesis",
        "if then", "therefore", "because",
    ]):
        return TaskCategory.REASONING

    # Simple factual questions
    if len(p) < 120 and any(kw in p for kw in [
        "who", "what is", "when", "where", "capital of",
        "definition", "meaning of", "define",
    ]):
    ...
    # Summarization
    if any(kw in p for kw in [
        "summarize", "summary", "tl;dr", "synopsis",
    ]):
        return TaskCategory.SUMMARIZATION

    # Extraction
    if any(kw in p for kw in [
        "extract", "find all", "list all", "pull out",
    ]):
        return TaskCategory.EXTRACTION

    # Creative
    if any(kw in p for kw in [
        "write a story", "poem", "creative", "tell me a",
        "write a tale", "narrative",
    ]):
